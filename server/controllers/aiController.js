...
const getAvailableModelName = async (apiKey) => {
  try {
    ...
    if (data.models && data.models.length > 0) {
      ...
      const modernFlash = generativeModels.find((m) => m.name.includes('gemini-3.6-flash'));
      if (modernFlash) return modernFlash.name;

      // 2. Second priority: any other active Flash model
      const anyFlash = generativeModels.find((m) => m.name.toLowerCase().includes('flash'));
    ...
    console.warn('[AI Model Discovery] Falling back to default model:', err.message);
  }
...
const analyzeLegalIssue = async (req, res) => {
  try {
    ...
    const cleanModelName = modelResource.startsWith('models/') ? modelResource : `models/${modelResource}`;
